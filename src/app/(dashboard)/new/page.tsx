...
import { Loader2, Zap, FileText, Hash, Briefcase, Mail, ArrowRight, Globe } from "lucide-react";
...
export default function NewPage() {
  ...
  useEffect(() => {
    async function loadCredits() {
      ...
      if (limit !== null) setCreditsLeft(Math.max(0, limit - (usage?.count ?? 0)));
    }
    void loadCredits();
  }, []);
        ...
        <form onSubmit={(e) => { e.preventDefault(); void handleSubmit(); }} className="space-y-6">
          ...
          <div>
            ...
            <div className="relative">
              ...
              />
              {valid && <div className="absolute right-3 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-emerald-400" />}
            </div>
          ...
          <div>
            ...
            <p className="mt-2 text-xs text-white/25">{TONES.find((t) => t.value === tone)?.desc}</p>
          </div>
